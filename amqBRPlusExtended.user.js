...
// @author       Mxyuki (Original) + xTsuSaKu (Extender)
...
function setup(){
    let loadInterval2 = setInterval(() => {
        if (typeof BattleRoyalShowEntry === "function") {
	...
	if (object.info.type === 'NameEntry') {
		entry = new BattleRoyalShowEntry(object.info.id, this.extractShowName(object.info), object.x, object.y, inventoryFull, this.inventoryFullMessageController, () => {
...
function BattleRoyalShowEntry(annId, name, x, y, inventoryFull, inventoryFullMessageController, updatePosition) {
    const template = $("#brShowEntryTemplate").html();
	this.$html = $(format(template));
	this.annId = annId;
	this.x = parseInt(x);
	this.y = parseInt(y);
	this.$html.popover({
...
BattleRoyalShowEntry.prototype.constructor = BattleRoyalShowEntry
...
BattleRoyalContainerEntry.prototype.showContent = function (entries) {
	if (this.active) {
		...
		entries.forEach(entry => {
			let $entry = $("<li class='brContainerEntry-" + entry.id + "'>" + entry.id + " | " + entry.name + '</li>');
			$entry.popover({
...
BattleRoyalContainerEntry.prototype.removeContentEntry = function (id) {
	...
	$entry.popover('disable').popover('hide');
	this.$containerContent.addClass('transitionDisable');
	$entry.remove();
	//Make it so new popovers aren't triggered strait away as this causes issue with bootstrap
	setTimeout(() => {
		this.$containerContent.removeClass('transitionDisable');
	}, 50);
};
...
let annCache = JSON.parse(localStorage.getItem("xtsk.brplus")) ?? {};

new Listener("new collected name entry", async (payload) => {
    let precision = annCache[payload.id]
    if(!precision){
        const annData = annCache[payload.id] ??  await fetch("https://cdn.animenewsnetwork.com/encyclopedia/api.xml?anime=" + payload.id).then(data => data.text());
        const parser = new DOMParser();
        const xmlDoc = parser.parseFromString(annData,"text/xml");
        precision = xmlDoc.querySelector("anime").getAttribute("precision");
        annCache[payload.id] = precision;
        localStorage.setItem("xtsk.brplus", JSON.stringify(annCache))
    }
    if(language == 0){
        if(payload.eng){
...
new Listener("battle royal spawn", async(payload) => {
